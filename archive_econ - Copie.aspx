...
<script runat="server">
	...
	'End If	
        ...
        CommandText = "select num_commande,dtcomm,societe,modele,ref_client,prix_pointfort,date_usine,montant_usine,nom_usine,date_tournee,heure_tournee,status,langue from commandes_portes"
        CommandText = CommandText + " left join clients_new on clients_new.codeclient = commandes_portes.num_client"
        ...
        CommandText = "select num_commande,dtcomm,societe,modele,ref_client,prix_pointfort,date_usine,montant_usine,nom_usine,date_tournee,heure_tournee,status,langue from commandes_portes "
        CommandText = CommandText + " left join clients_new on clients_new.codeclient = commandes_portes.num_client"
        CommandText = CommandText + " LEFT OUTER JOIN signature ON commandes_portes.num_client = signature.CodeClient"
        CommandText = CommandText + " where signature.Codevalideur = '" & filterValue & "'"
        ...
        SqlDataSource1.SelectCommand = CommandText
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
		  ...
		  Dim Lblcde As Label
            ...
            'If ToolRecup.get_order_info(Lblcde.Text, "environnement").ToLower = "default" Then
                'e.Row.Cells(13).Text = "<a href='" & "http://econ.fichet-pointfort.fr/commandes/CEW" & Lblcde.Text.Trim.PadLeft(7, "0") & "_TC.HTML" & "' target='_BLANK'><img src='images/cle.png'  border='0'></a>"
                ...
                e.Row.Cells(14).Text = "<a href='" & cUrl &  tabName(0) & "/" & tabName(1) & "/CEW" & Lblcde.Text.Trim.PadLeft(7, "0") & "_TC.HTML" & "' target='_BLANK'><img src='images/cle.png'  border='0'></a>"
            'Else
                'e.Row.Cells(13).Text = "<a href='" & "http://econ.fichet-pointfort.fr/commandes_test/CEW" & Lblcde.Text.Trim.PadLeft(7, "0") & "_TC.HTML" & "' target='_BLANK'><img src='images/cle.png'  border='0'></a>"
             '   e.Row.Cells(13).Text = "<a href='" & sPATH & "/commandes_test/" & tabName(0) & "/" & tabName(1) & "/CEW" & Lblcde.Text.Trim.PadLeft(7, "0") & "_TC.HTML" & "' target='_BLANK'><img src='images/cle.png'  border='0'></a>"
            'End If
    ...
    Private Function affiche_commande(ByVal wCde As String) As Boolean
        ...
        cSql = " SELECT * FROM commandes_portes" & _
                            " INNER JOIN portes_confirmation ON commandes_portes.produit_url = portes_confirmation.produit_url " & _
                                " WHERE     (portes_confirmation.confirmation = 1) and (num_commande=" & wCde & ") AND commandes_portes.status in ('0','1','2','3','4','5')"
...
<asp:Content ContentPlaceHolderID="e_body" runat="server">
    ...
    <p align="left">
        ...
        <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="" ProviderName="System.Data.SqlClient"
            SelectCommand="select num_commande,dtcomm,societe,modele,ref_client,prix_pointfort,date_usine,montant_usine,nom_usine,date_tournee,heure_tournee,status,langue from commandes_portes 

        left join clients_new
		on clients_new.codeclient = commandes_portes.num_client
        where num_client = @client order by num_commande desc">
            <SelectParameters>
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="" ProviderName="System.Data.SqlClient"
            SelectCommand="select num_commande,dtcomm,societe,modele,Ref_valideur,prix_pointfort,date_usine,montant_usine,nom_usine,date_tournee,heure_tournee,status,langue from commandes_portes 
         LEFT OUTER JOIN signature ON commandes_portes.num_client = signature.CodeClient
        left join clients_new on clients_new.codeclient = commandes_portes.num_client
        where signature.Codevalideur = @client order by num_commande desc">
            <SelectParameters>
                <asp:SessionParameter SessionField="client" Name="client" Type="string" />
            </SelectParameters>
        </asp:SqlDataSource>
