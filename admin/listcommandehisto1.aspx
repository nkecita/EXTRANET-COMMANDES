	...
	Sub RechercherParNumCmd(sFiltre as String)
      ...
      SqlDataSource3.ConnectionString = System.Configuration.ConfigurationManager.AppSettings("connectionString")
		...
		If Session("filtre") <> "" Then 
			SqlDataSource3.SelectCommand = "SELECT [num_commande], [dtcomm],hrcomm,[societe], [num_client], [ref_client], [delai],  [prix_valideur],[prix_pointfort], [modele],status FROM [commandes_portes] where 1=1" & " AND Environnement ='" & cEnviron & "'"  & Session("filtre") & "  "& IIf (String.IsNullOrEmpty(sFiltre), String.Empty, " AND num_commande like '" & sFiltre &"'") &" order by num_commande desc "
		Else
			SqlDataSource3.SelectCommand = "SELECT [num_commande], [dtcomm],hrcomm,[societe], [num_client], [ref_client], [delai],  [prix_valideur],[prix_pointfort], [modele],status FROM [commandes_portes] where Environnement ='" & cEnviron & "' " & IIf (String.IsNullOrEmpty(sFiltre), String.Empty, " AND num_commande like '" & sFiltre &"'") &"  order by num_commande desc "
		End If
